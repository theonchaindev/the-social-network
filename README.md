...
`/brand` is the brand kit: mark, palette, type, downloadable graphics and tweet
copy with a working-ticker field. The PNGs are generated by `scripts/brand.py`
(PIL, the site's own fonts fetched from google/fonts into `scripts/fonts/`,
which is gitignored); rerun it after changing `lib/brand.ts`.
