...
`/brand` is the brand kit: palette, type, downloadable plates and tweet copy
with a working-ticker field. **There is no logo.** The brand is the two things
the site renders — rain on glass and the avatar network — and `scripts/brand.py`
ports both from their real sources: the refraction maths from
`components/three/RainGlass.tsx`, the graph layout and avatar tiles from
`lib/graph.ts` and `lib/avatars.ts`. A plate and the page it came from cannot
drift apart. Needs numpy and the site's own fonts, fetched into the gitignored
`scripts/fonts/`; rerun after changing `lib/brand.ts`.

One deviation from the shader, deliberate: the pane is given a real Gaussian
blur rather than the shader's five averaged samples, because at print size that
average is too subtle for the droplets to read against.

`/admin` is password-gated (`proxy.ts` checks a JWT cookie; `ADMIN_PASSWORD`
and `ADMIN_SECRET` in env). It shows wallet balances, pool fee buckets,
holders with their projected share, and round history, and can run a payout
round: claim the creator stream, snapshot holders, pay pro rata. That needs
`PAYOUT_WALLET_KEY` on the server — the payout wallet's key only, which holds
nothing but holders' money between rounds. Without it the page is read-only.
Never put the dev wallet's key anywhere near this.
